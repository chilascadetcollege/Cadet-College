...
const initScript = () => {
        ...
        facultyData.forEach((member, index) => {
            ...
            card.innerHTML = 
                <div class="faculty-img-wrapper" style="width:120px; height:120px; border-radius:50%; margin: 0 auto 1.5rem auto; overflow:hidden; border: 3px solid var(--accent); box-shadow: 0 0 20px rgba(245, 166, 35, 0.3);">
                    <img src="" alt="" style="width:100%; height:100%; object-fit:cover; filter: contrast(1.1) saturate(1.2);">
                </div>
                <h3 class="faculty-name"></h3>
                <p class="faculty-role"></p>
                <p class="faculty-desc"></p>
            ;
    ...
    const playWelcomeGreeting = (lang) => {
        ...
        const playOnlineVoice = () => {
            ...
            if (lang === "en") {
                audioUrl = https://translate.googleapis.com/translate_tts?client=gtx&ie=UTF-8&tl=en&q=;
            } else {
                audioUrl = https://translate.googleapis.com/translate_tts?client=gtx&ie=UTF-8&tl=ur&q=;
            }
...
# We write this using utf-8 directly but bypassing console encoding mismatches
with io.open("script_temp.js", "w", encoding="ascii") as f:
    f.write(js_code.encode("ascii", "backslashreplace").decode("ascii"))
